...
from flask-jwt-flask_jwt_extended import jwt_required
from api import db, Tag
...
def create_blog():
    ...
    for tag in data['tags']:
        present_tag = Tag.quary.filter_by(name=tag).first()
        if (present_tag):
            present_tag.blog_associated.append(new_blog)
        else:
            new_tag = Tag(name=tag)
            new_tag.blog_associated.append(new_blog)
